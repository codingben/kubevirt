...
func executeOperations(commands []AgentCommand, con cli.Connection, agentStore *AsyncAgentStore, domainName string) {
	for _, command := range commands {
		...
		case GET_FSFREEZE_STATUS:
			if status, err := getFilesystemFreezeStatus(con, domainName); err == nil {
				agentStore.Store(GET_FSFREEZE_STATUS, status)
			}
		case GET_FILESYSTEM:
			if fileSystem, err := getFilesystem(con, domainName); err == nil {
				agentStore.Store(GET_FILESYSTEM, fileSystem)
			}
		case GET_AGENT:
			if agentInfo, err := getAgentInfo(con, domainName); err == nil {
...
func getFilesystemFreezeStatus(con cli.Connection, domainName string) (api.FSFreeze, error) {
	// FSFREEZE_STATUS is not exposed in libvirt API
	result, err := con.QemuAgentCommand(`{"execute":"`+string(GET_FSFREEZE_STATUS)+`"}`, domainName)
	if err != nil {
...
func getFilesystem(con cli.Connection, domainName string) ([]api.Filesystem, error) {
	result, err := con.QemuAgentCommand(`{"execute":"`+string(GET_FILESYSTEM)+`"}`, domainName)
	if err != nil {
		return []api.Filesystem{}, err
	}

	filesystems, err := parseFilesystem(result)
	if err != nil {
...
func getAgentInfo(con cli.Connection, domainName string) (AgentInfo, error) {
	// Consider if getting AGENT info is needed
	result, err := con.QemuAgentCommand(`{"execute":"`+string(GET_AGENT)+`"}`, domainName)
	if err != nil {
